...
def _clean_customer(record):
    if not record:
        return None
    customer = dict(record)
    for key in SIM_CUSTOMER_COLUMNS:
        customer.setdefault(key, "")
    return customer
...
def get_registered_sim_customer(sim_id):
    if not sim_id:
        return None
    data = (
        get_supabase()
        .table("sim_customers")
        .select("*")
        .eq("sim_id", sim_id.strip())
        .limit(1)
        .execute()
        .data
    )
    return _clean_customer(data[0]) if data else None
...
def search_sim_customers(keyword):
